...
root = Tk()
host = ''  # what address is the server listening on
...
while running:
    ...
    for s in inputready:  # check each socket that select() said has available data
        ...
        if s == server:  # if select returns our server socket, there is a new
            ...
            label = Label(root, text='new client added %s' % str(address))

        else:
            # select has indicated that these sockets have data available to recv
            data = s.recv(maxsize)
            if data:
                label = Label(root, text='%s received from %s' % (data, s.getsockname()))
